...
| `assets/examples/` | 示例 spec、remote fixtures，以及 refined template 输入样例。 |
| `evals/` | 仓库内 skill-effectiveness 用例，用于 workflow 与 remote-validation 回归检查。 |
| `RELEASE_RECEIPT.json` | 导入的 `v0.2.0` 发布包来源记录。 |
...
`v0.2.0` 这一版把工作流包面进一步扩展到了 refined RTL templates、use-case bundles、repo-local evals、远程 toolchain 选择辅助，以及按 dependency、toolchain、effectiveness 拆分的 smoke gates。
