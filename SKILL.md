...
The same skill also exposes two local helper scripts for independent VG review and testbench scaffold generation without widening the skill beyond Verilog-2001. Canonical Python implementations live under `scripts/python/<function>/`, and command-line entrypoints must invoke those modules directly.

- `scripts/python/quality/verilog_lint.py` for the lint view of unified VG results
...
- three-segment FSMs using `state_current`, `state_next`, and `ST_*`, with explicit next-state default hold, full `if / else if / else` closure, and pure leading comments above `case(state_current)` state/default branches;
- fixed Chinese region banners and Chinese-first semantic comments that are entity-specific, non-placeholder, and not exact or near-duplicate reuse of another entity comment, plus controlled spacing when a pure comment introduces an `assign` subgroup.
- when parameter checks are required, place the `parameter_check` region as the final internal region before `endmodule`, emit it only for concrete validated constraints, and keep module instances immediately before it.
- human-readable Verilog `$display` text must start with ` > INFO: [Verilog]`, ` > WARNING: [Verilog]`, or ` > ERR: [Verilog]`; machine-readable transcript tags such as `VERILOG-GEN-RESULT` stay exempt.
...
The deliverable gate v2 aggregates one formatter AST pass, all 121 active VG rules, comment checks, and rulebook consistency. Strict mode is the default and requires `errors == 0` and `strict_warnings == 0`. The same unified VG quality gate remains available for focused debugging:
...
9. Apply the unified Verilog quality catalog in `references/rules/verilog-quality-gates.md` and `assets/verilog_quality_gates.json`: report only VG identifiers; execute all 121 active rules with no reserved entries; fail closed for `BLOCKER` non-pass states, and block `WARNING` non-pass states only in strict mode. Use `run_verilog_quality_gate(...)` as the single public entry and expose `vg_catalog_version`, `vg_rule_summary`, and `vg_rule_results` in schema v2 reports.
...
- Load `references/rules/verilog-quality-gates.md` when changing generated RTL syntax constraints, VG evaluators, prompt guidance, result states, or the `assets/verilog_quality_gates.json` catalog.
