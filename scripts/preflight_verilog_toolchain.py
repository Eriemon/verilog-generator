...
PATH_SKILL_ROOT = Path(__file__).resolve().parents[1]  # runtime 包和默认配置所在目录
...
def main(argv: list[str] | None = None) -> int:
    ...
    from runtime.verilog_generator.validation import READINESS_LEVELS, require_readiness
    ...
    from runtime.verilog_generator.config import load_settings
...
def build_report(dict_settings: dict[str, Any], str_readiness: str) -> dict[str, Any]:
    ...
    from runtime.verilog_generator.config import policy_setting, remote_setting
    from runtime.verilog_generator.remote_selection import (
        remote_runtime_config_relpath,
        resolve_confirmed_remote_server,
    )
    from runtime.verilog_generator.validation import readiness_at_least
