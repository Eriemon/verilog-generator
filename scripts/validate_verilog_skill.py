...
def is_source_repository_layout() -> bool:
    """判断当前 validate 脚本是否运行在带完整 docs/tests/smoke 的源码仓库中。

    :param: 此函数不接收外部业务参数。
    :return: 返回布尔值；True 表示“判断当前 validate 脚本是否运行在带完整 docs/tests/smoke 的源码仓库中。”对应条件命中。
    """

    # 源码仓库要求 skill 位于 `<repo>/skills/<name>` 标准布局。
    path_expected_skill = PROJECT_ROOT / "skills" / SKILL_ROOT.name  # 源码仓库预期的 skill 位置

    # docs/tests/smoke 三组标记共同表示完整仓库治理上下文存在。
    tuple_repo_markers = (
        PROJECT_ROOT / "docs" / "handoff" / "HANDOFF.md",  # 交接文档是源码仓库治理标记
        PROJECT_ROOT / "smoke" / "run_smoke.py",  # smoke 根脚本只存在于源码仓库
...
def load_settings(settings_path: Path) -> dict[str, Any]:
    ...
    from runtime.verilog_generator.config import load_settings as runtime_load_settings
...
def path_setting(settings: dict[str, Any], key: str) -> Path:
    ...
    from runtime.verilog_generator.config import path_setting as runtime_path_setting
...
def build_validation_context(namespace_args: argparse.Namespace) -> ValidationContext:
    ...
    from runtime.verilog_generator.config import load_settings, path_setting
...
def run_optional_external_audit(validation_context: ValidationContext) -> None:
    ...
    from runtime.verilog_generator.config import path_setting
...
def run_self_contained_gates(validation_context: ValidationContext) -> Path:
    ...
    # markdown ASCII 约束保障 skill 安装安全。
    ...
    list_eval_skill_command = [  # 产出效果评估报告并禁止状态残留的本地评估命令
        ...
        "-m",  # 以模块方式启动 runtime CLI
        "runtime.verilog_generator",  # Verilog 生成器命令行模块入口
...
def run_compile_and_optional_regression(validation_context: ValidationContext) -> None:
    ...
    # 安装包三大代码域必须可编译，源码仓库与安装副本只在路径锚点上不同。
    list_compile_targets = [
        str(SKILL_ROOT / "runtime"),  # runtime 编译目标
        str(SKILL_ROOT / "integration"),  # 宿主适配层编译目标
        str(SKILL_ROOT / "scripts"),  # 随包命令脚本编译目标
    ]  # compileall 默认目标

    # repo regression 打开时补充仓库根 smoke 和 tests。
    if validation_context.namespace_args.with_repo_regression:

        # 安装副本没有仓库级 tests/smoke，显式要求时必须指出使用边界。
        ...
        # tests/smoke 不属于安装包主体，但源码仓库回归需要覆盖。
        list_compile_targets.extend([str(PROJECT_ROOT / "smoke"), str(PROJECT_ROOT / "tests")])
    ...
    # smoke/run_smoke.py 使用与当前验证一致的 settings。
    run(
        [
            sys.executable,
            str(PROJECT_ROOT / "smoke" / "run_smoke.py"),
            "--settings",
...
# run_verilog_cli 用统一入口调用 runtime.verilog_generator CLI。
def run_verilog_cli(*str_args: str) -> None:
    """执行一次 runtime.verilog_generator CLI 命令。

    :param *str_args: 执行一次 runtime.verilog_generator CLI 命令。 阶段使用的 `str_args` 输入。
    :return: 不返回业务值；执行完成即表示“执行一次 runtime.verilog_generator CLI 命令。”对应步骤未发现阻断。
    """

    # 命令前缀固定为当前 Python 解释器和 runtime 模块。
    list_command = [sys.executable, "-m", "runtime.verilog_generator", *str_args]  # runtime 模块 CLI 命令

    # 子进程工作目录按源码仓库/安装副本布局自动选择。
    run(list_command, cwd=validation_workspace_root())
...
def run_verify_existing(request: VerifyExistingRequest) -> None:
    ...
    # 统一通过 runtime.verilog_generator CLI 入口运行。
    run_verilog_cli(*list_command_args)
...
def build_remote_validation_command(
    ...
    *,
    ...
    # 远程验证脚本既支持源码仓库，也支持安装副本，因此使用 skill 内绝对路径。
    list_command = [
        sys.executable,  # 远程 gate 子进程沿用当前解释器
        str(SKILL_ROOT / "scripts" / "remote_validate_verilog_skill.py"),  # 当前 skill 内的远程验证入口
        "--settings",  # 远程验证 settings 参数名
        str(settings_path),  # 本轮 validate 使用的 settings 文件
    ]  # 传给 remote_validate_verilog_skill.py 的基础命令
...
def resolve_remote_server(settings: dict) -> str | None:
    ...
    from runtime.verilog_generator.remote_selection import resolve_confirmed_remote_server
...
def resolve_required_remote_validation_state(
    ...
    *,
    ...
    from runtime.verilog_generator.config import remote_setting
...
def _is_transient_audit_artifact_failure(output: str) -> bool:
    ...
    # 只有同时出现 FileNotFoundError 和已知临时产物路径时才允许清理后重试。
    if "FileNotFoundError" not in output:

        # 缺少瞬态缺失异常时直接判定为不可重试。
        return False

    # 允许重试的路径只覆盖 validate 会主动清理的 smoke 目录和 Python 缓存目录。
    return "_smoke_runs" in output or "__pycache__" in output
...
# verify_markdown_ascii 防止 Markdown 文档包含安装环境不稳定的非 ASCII 字符。
def verify_markdown_ascii() -> None:
    """确认 skill 包内 Markdown 文件保持 ASCII-only。

    :param: 此函数不接收外部业务参数。
    :return: 不返回业务值；执行完成即表示“确认 skill 包内 Markdown 文件保持 ASCII-only。”对应步骤未发现阻断。
    :raises AssertionError: 当“确认 skill 包内 Markdown 文件保持 ASCII-only。”阶段发现配置、产物或执行结果不满足门禁要求时抛出。
    """

    # 公开仓库允许保留双语中文 README，其余 Markdown 继续保持 ASCII-only。
    set_non_ascii_markdown_allowlist = {"README-CN.md"}  # 允许公开保留非 ASCII 的 Markdown 文件
    ...
    for path_file in iter_skill_files():
        ...
        # 仓库公共中文 README 不属于安装安全阻断面。
        if str_rel in set_non_ascii_markdown_allowlist:
...
def verify_skill_design_documents() -> None:
    ...
    path_standards = SKILL_ROOT / "references" / "skill-standards.md"  # 设计标准文档路径

    # standards 文件必须存在。
    if not path_standards.exists():

        # 标准文档缺失会让发布包缺少评估约束，沿用旧错误文本。
        raise AssertionError("> ERR: [Python] references/skill-standards.md is required.")
    ...
    for str_marker in PATTERN_NAMES:
        ...
        if str_marker not in str_standards_text:
            ...
            raise AssertionError(f"> ERR: [Python] references/skill-standards.md must mention {str_marker!r}.")
...
def verify_skill_eval_assets() -> None:
    ...
    # eval-skill runtime 目录承载 CLI 评估入口和 fixture 执行套件。
    path_eval_runtime_dir = SKILL_ROOT / "runtime" / "verilog_generator"  # 评估 runtime 包目录

    # 必需 eval 文件覆盖 CLI 评估入口和 fixture 执行套件，缺任一项都会让效果评估无法运行。
    list_required_eval_paths = [
        path_eval_runtime_dir / "evaluation.py",  # eval-skill CLI 评估入口
        path_eval_runtime_dir / "eval_suite.py",  # eval fixture 执行套件
    ]  # 效果评估运行时必须随包发布的文件
...
# _allowed_smoke_script_dependency_line 解释 smoke 主脚本的工具链路径样例。
def _allowed_smoke_script_dependency_line(str_line: str) -> bool:
    """判断 smoke/run_smoke.py 中的 legacy 术语是否属于工具链发现逻辑。

    :param str_line: 正在检查的单行文本。
    :return: 返回布尔值；True 表示“判断 smoke/run_smoke.py 中的 legacy 术语是否属于工具链发现逻辑。”对应条件命中。
    """
...
# _allowed_smoke_dependency_gate_line 解释 smoke 依赖 gate 的厂商和 skill 标识。
def _allowed_smoke_dependency_gate_line(str_line: str) -> bool:
    """判断 smoke/dependency_gates.py 中的 legacy 术语是否属于依赖 schema 检查。

    :param str_line: 正在检查的单行文本。
    :return: 返回布尔值；True 表示“判断 smoke/dependency_gates.py 中的 legacy 术语是否属于依赖 schema 检查。”对应条件命中。
    """
...
# _allowed_toolchain_gate_line 解释 smoke 工具链 gate 的后端字段。
def _allowed_toolchain_gate_line(str_line: str) -> bool:
    """判断 smoke/toolchain_gates.py 中的 legacy 术语是否属于工具链 gate。

    :param str_line: 正在检查的单行文本。
    :return: 返回布尔值；True 表示“判断 smoke/toolchain_gates.py 中的 legacy 术语是否属于工具链 gate。”对应条件命中。
    """
...
# _allowed_existing_rtl_refinement_line 解释 existing RTL refinement 的 SV/SVA 支持。
def _allowed_existing_rtl_refinement_line(str_line: str) -> bool:
    """判断 existing_rtl_refinement.py 中的 legacy 术语是否属于 SV/SVA 兼容说明。

    :param str_line: 正在检查的单行文本。
    :return: 返回布尔值；True 表示“判断 existing_rtl_refinement.py 中的 legacy 术语是否属于 SV/SVA 兼容说明。”对应条件命中。
    """

    # refinement allowlist 需要识别 SVA property 示例。
    str_lower_line = str_line.lower()  # refinement 文档折叠文本
    ...
    # existing RTL refinement 允许提到 SV 或 SVA 语义约束。
...
def _build_dependency_term_handlers() -> dict[str, Callable[[str], bool]]:
    ...
    return {
        ...
        "references/integration.md": _allowed_integration_dependency_line,  # 集成说明里的测试平台术语
        "references/configuration.md": _allowed_configuration_dependency_line,  # configuration 依赖示例
        "scripts/validate_verilog_skill.py": _allowed_validate_script_dependency_line,  # 本脚本自检术语
        "scripts/manage_skill_dependencies.py": _allowed_dependency_manager_line,  # 依赖管理脚本术语
        "scripts/tb_generator.py": _allowed_tb_generator_dependency_line,  # testbench 语言选项
        "runtime/verilog_generator/cli_generation_commands.py": _allowed_cli_generation_dependency_line,  # facade 字段
        "runtime/verilog_generator/cli_parser.py": _allowed_cli_parser_dependency_line,  # CLI 兼容参数
        "scripts/remote_validate_verilog_skill.py": _allowed_remote_validate_dependency_line,  # 远程后端字段
        "smoke/run_smoke.py": _allowed_smoke_script_dependency_line,  # smoke 工具链发现
        "smoke/dependency_gates.py": _allowed_smoke_dependency_gate_line,  # smoke 依赖架构检查术语
        "smoke/toolchain_gates.py": _allowed_toolchain_gate_line,  # smoke 工具链门禁术语
        "evals/evals.json": _allowed_eval_dependency_line,  # 效果评估用例描述术语
        "runtime/verilog_generator/existing_rtl_refinement.py": _allowed_existing_rtl_refinement_line,  # RTL 精化术语
        "runtime/verilog_generator/skill_effectiveness.py": _allowed_skill_effectiveness_dependency_line,  # 评估覆盖项
        "runtime/verilog_generator/verify_repair.py": _allowed_verify_repair_dependency_line,  # verify-repair 语言字段
    }
...
def verify_dependency_schema(settings: dict) -> None:
    ...
    from runtime.verilog_generator.config import fpga_developer_routing_settings, skill_dependency_settings
...
def verify_hardcoded_paths() -> None:
    ...
    set_allowed = {
        ...
        "references/configuration.md",  # configuration 文档可展示路径配置示例
...
def verify_no_ref_dependencies() -> None:
    ...
    if is_source_repository_layout():
        ...
        list_active_paths.extend(
            [
                ...
                PROJECT_ROOT / "smoke" / "run_smoke.py",  # smoke 主脚本不得依赖 ref 临时输入
            ]
        )
    ...
    # scripts 下的发布/验证脚本同样不应依赖临时 ref 输入。
    list_active_paths.extend(sorted((SKILL_ROOT / "scripts").glob("*")))
...
def remove_inside_skill(path: Path) -> None:
    ...
    # 目录使用带重试的 rmtree，文件直接 unlink，保持清理副作用最小。
...
def iter_skill_files() -> list[Path]:
    ...
    set_ignored_parts = {"__pycache__", "_smoke_runs", "dist", "reports"}  # skill 文件枚举时跳过的路径片段

    # release receipt 属于打包元数据，不应参与源码 hygiene 扫描。
    set_ignored_names = {"RELEASE_RECEIPT.json", ".git"}  # skill 文件枚举时跳过的生成元数据文件名
...
def run(command: list[str], *, cwd: Path, allow_failure: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    # stderr 继续写入 stderr，避免错误信息被普通日志吞没。
    if completed_process_result.stderr:

        # 子命令 stderr 保持错误通道，便于调用方按流区分诊断。
        sys.stderr.write(completed_process_result.stderr)
