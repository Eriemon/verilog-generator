...
# argparse、sys 和 Path 共同完成参数解析、运行时导入路径和退出码处理。
import argparse
import sys
from pathlib import Path

# 脚本位于 scripts 目录，上一层就是可加入 sys.path 的 skill 包根。
PATH_SKILL_ROOT = Path(__file__).resolve().parents[1]  # CLI 直运行时的 skill 包根
...
def _default_markdown_report_path() -> Path:
    ...
    from runtime.verilog_generator.config import project_root
