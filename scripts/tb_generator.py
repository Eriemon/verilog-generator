...
def split_top_level_commas(text: str) -> list[str]:
    ...
    for int_index, str_char in enumerate(text):
        ...
        # 顶层逗号标记一个完整端口片段结束。
            ...
            # 截取当前端口片段并去掉外围空白。
            str_part = text[int_start:int_index].strip()  # 当前端口片段
    ...
    # 处理最后一个逗号之后的尾部片段。
    str_tail = text[int_start:].strip()  # 端口列表尾部片段

    # 尾部非空时追加到结果。
    if str_tail:

        # 保留最后一个端口片段。
        list_parts.append(str_tail)
...
def _append_header_lines(
    ...
    _append_many(
        ...
        (
            ...
            "// Reference vector hash placeholder: ERIE_VECTOR_HASH <pending>",
            "`timescale 1ns / 1ps",
            "",
            f"module {tb_name};",
            "",
