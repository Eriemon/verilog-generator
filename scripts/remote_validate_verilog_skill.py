...
PATH_SKILL_ROOT = Path(__file__).resolve().parents[1]  # 包含 runtime、scripts 与 config 的 skill 根目录
...
def load_settings(path_settings: Path) -> dict[str, Any]:
    ...
    from runtime.verilog_generator.config import load_settings as func_load_settings
...
def remote_setting(dict_settings: dict[str, Any], str_key: str) -> str:
    ...
    from runtime.verilog_generator.config import remote_setting as func_remote_setting
...
def remote_runtime_settings_relpath() -> str:
    ...
    from runtime.verilog_generator.config import (
...
def require_workspace_root(*, purpose: str) -> Path:
    ...
    from runtime.verilog_generator.workspace import require_workspace_root as func_require_workspace_root
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # 远端 runtime 配置文件也必须相对 workdir，避免上传到任意绝对路径。
        str_remote_runtime_config = require_remote_relative_path(  # 远端 runtime 配置相对路径
    ...
    *,
    ...
    try:
        ...
        list_request_paths.append(
            request_and_run(
                remote_context,
                "request-mkdir",
                ["--path", str_remote_parent, "--reason", "prepare Verilog skill validation directory"],
            )
        )
        ...
        list_request_paths.append(
            request_and_run(
                ...
                [
                    "--local",
                    str(path_package_root / "erie-verilog-generator"),
                    "--remote",
                    str_remote_skill,
                    "--reason",
        ...
        str_command = remote_validation_command(  # 远端 bash 验证脚本
            str_remote_skill,  # 远端上传后的 skill 工作区
            str_remote_python,  # 远端 Python 命令
            cleanup_outputs=cleanup_remote,  # 是否清理远端 smoke 输出
            toolchain_selection=dict_remote_runtime["toolchain"],  # 已确认工具链选择
            remote_runtime_config_path=str_remote_runtime_config,  # Vivado 多版本提示中的配置文件路径
        )
    ...
    # 清理阶段无论远端命令成功与否都要执行。
        ...
        # 默认 retained 策略下报告远端证据目录。
            ...
            # retained 父目录是用户后续查看的入口。
            print(f"> INFO: [Python] remote_retained: {str_remote_parent}")

            # retained skill 目录包含 smoke 产物和 validation.json。
            print(f"> INFO: [Python] remote_skill_retained: {str_remote_skill}")
...
def download_remote_runtime_config(
    ...
    # 下载副本放在 reports/downloads 下，便于失败时人工查看。
    path_local_copy = (  # 下载到本地 reports/downloads 的 runtime 配置副本
        remote_context.path_helper.resolve().parents[1] / "reports" / "downloads" / "verilog.remote.download.json"  # helper 项目 reports/downloads 副本
    )

    # 确保下载目录存在。
    path_local_copy.parent.mkdir(parents=True, exist_ok=True)
...
# stage_package 复制 skill 主体和 smoke 目录到临时上传包。
def stage_package(path_helper: Path, str_run_id: str) -> Path:
    ...
    # smoke 目录位于 staging 工作区根。
    path_staged_smoke = path_target / "smoke"  # 远端回归所需的仓库根 smoke 副本
    ...
    # 复制仓库根 smoke 目录。
    shutil.copytree(PATH_PROJECT_ROOT / "smoke", path_staged_smoke, ignore=obj_copytree_ignore_patterns)
...
def summarize_remote_run(
    ...
    str_execute_validation_json = remote_join(  # remote_execute 摘要读取的原始 validation 报告
        ...
        "_smoke_runs/remote_execute/attempt-001/validation.json",  # attempt-001 保存的执行门禁报告
    )
    ...
    str_execute_rtl_path = remote_join(  # 摘要中记录 erie_adapter.v 的 retained 地址
        ...
        "_smoke_runs/remote_execute/attempt-001/rtl/generated/rtl/erie_adapter.v",  # 适配器源码相对位置
    )
    ...
    str_execute_testbench_path = remote_join(  # 失败复盘入口中的仿真平台文件名
        ...
        "_smoke_runs/remote_execute/attempt-001/rtl/generated/tb/erie_adapter_tb.v",  # 仿真平台相对位置
    )
    ...
    dict_fixture_summary = download_json_optional(  # 三项小用例回归的聚合结果
        ...
        remote_join(str_remote_skill, "_smoke_runs/remote_fixtures/summary.json"),  # 三项 fixture 汇总源文件
...
{str_py} -m compileall -q \
  ...
  smoke
{str_py} smoke/run_smoke.py --settings skills/erie-verilog-generator/config/defaults.json
{str_rtl_md_snippet}
...
{str_py} -m runtime.verilog_generator run-workflow \
...
{str_py} -m runtime.verilog_generator validate \
  --spec skills/erie-verilog-generator/assets/examples/rtl_erie_verilog_spec.json \
...
PY
__PY__ -m runtime.verilog_generator eval-skill \
