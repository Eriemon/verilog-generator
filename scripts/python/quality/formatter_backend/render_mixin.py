...
class RenderMixin:
    ...
    def _render_module(self, context: ModuleRenderContext) -> str:
        ...
        if context.include_header:
            ...
            # 标准文件头和 module 声明之间固定保留一个空行和一条中文功能说明。
            str_module_purpose_comment = self._resolve_module_purpose_comment(context)  # header 后 module 前的中文功能说明

            # 只有说明正文非空时，才写回固定的模块功能说明合同。
            if str_module_purpose_comment:

                # 中文修订历史最后一行之后固定保留一个空行。
                list_lines.append("")

                # 模块功能说明统一由 renderer 贴上注释前缀，避免依赖 leading_comments 旁路。
                list_lines.append(f"// {str_module_purpose_comment}")
    ...
    # 在 include_header=True 路径下解析或推断模块功能中文说明。
    def _resolve_module_purpose_comment(self, context: ModuleRenderContext) -> str:
        """
        返回当前 module 应输出的中文功能说明正文。

        :param context: 单 module 渲染上下文。
        :return: 不含 `//` 前缀的模块功能说明正文。
        """

        # 显式解析出的模块功能说明优先保留，避免覆盖用户手写语义。
        if context.header_metadata and context.header_metadata.module_purpose_comment.strip():

            # 历史来源若仍保留 `//` 前缀，这里统一裁掉，由 renderer 负责贴回。
            return context.header_metadata.module_purpose_comment.strip().lstrip("/").strip()

        # 缺失显式说明时，根据当前结构推断一条非模板化中文说明。
        return self._infer_module_purpose_comment(context)

    # 根据端口、过程块和实例结构推断中文模块功能说明。
    def _infer_module_purpose_comment(self, context: ModuleRenderContext) -> str:
        """
        基于当前 module 结构推断中文功能说明。

        :param context: 单 module 渲染上下文。
        :return: 推断得到的不含注释前缀的中文说明正文。
        """

        # list_fragments 依次累积协议、控制与桥接职责短语。
        list_fragments: list[str] = []  # 模块功能说明短语片段

        # 协议或适配提示优先从 module 名和端口名里抽取。
        str_protocol_hint = self._module_protocol_hint(context.module_name, context.ports)  # 协议/接口提示词

        # 命中协议时把它放在句首，避免退化成空洞模块名复述。
        if str_protocol_hint:

            # 协议提示词统一转成“XX接口”句首片段。
            list_fragments.append(f"{str_protocol_hint}接口")

        # 全局 always 块体现时序、状态机或控制职责。
        list_all_always_blocks = self._collect_body_items_recursive(context.body_items, "always")  # 全 module always 过程块

        # 全局 assign 体现组合连线和桥接职责。
        list_all_assigns = self._collect_body_items_recursive(context.body_items, "assigns")  # 用于推断组合连线与桥接职责的 continuous assign 清单

        # 实例块体现子模块协同、调度或桥接职责。
        list_all_instance_blocks = self._collect_body_items_recursive(context.body_items, "instances")  # 全 module 实例块

        # 只要 always header 里显式出现 state，就优先按状态机控制类模块表述。
        bool_has_state_machine = any(  # always 头部是否显式暴露 state 关键字
            "state" in self._always_block_header_text(always_block).lower()  # 单个 always 头部里的 state 关键字判断
            for always_block in list_all_always_blocks  # always 头部里的状态机线索
        )  # 当前 module 是否存在显式状态机线索

        # output_internal_names 非空通常代表存在显式输出桥接路径。
        bool_has_output_bridges = bool(context.output_internal_names)  # 当前 module 是否存在输出桥接关系

        # 优先区分状态机控制、一般时序控制和纯组合连线三种主职责。
        if bool_has_state_machine:

            # 状态机类模块优先强调状态控制职责。
            list_fragments.append("状态机控制")

        # 没有显式 state 线索但存在 always 时，职责降级为一般时序控制。
        elif list_all_always_blocks:

            # 其余含 always 的模块至少承担时序控制职责。
            list_fragments.append("时序控制")

        # 既没有状态机也没有 always 时，组合连线成为最直接的模块职责。
        elif list_all_assigns:

            # 没有 always 时，用组合连线职责描述更贴近真实结构。
            list_fragments.append("组合连线")

        # 实例化路径存在时，再补充子模块协同语义。
        if list_all_instance_blocks:

            # 子模块实例通常对应协同、调度或桥接结构。
            list_fragments.append("子模块协同")

        # 输出桥接职责优先于泛化信号桥接职责。
        if bool_has_output_bridges:

            # 显式 output bridge 是多数结构化生成 RTL 的主要职责之一。
            list_fragments.append("输出桥接")

        # 仅存在连续赋值时，用更宽泛的信号桥接描述补足职责。
        elif list_all_assigns:

            # 没有输出桥接时，仍保留更泛化的信号桥接职责表述。
            list_fragments.append("信号桥接")

        # 完全没有结构线索时，仍需给出非空洞的处理职责。
        if not list_fragments:

            # 最保守兜底也必须表达“接口/信号处理”，避免退化成模板句。
            list_fragments.append("接口信号处理")

        # 统一用“模块”收束整句，形成单行中文功能说明。
        return "".join(list_fragments) + "模块"

    # 统一提取 always 块头文本，兼容结构化模型和极端兜底字符串。
    def _always_block_header_text(self, always_block: AlwaysBlock | object) -> str:
        """
        返回 always 块可用于语义推断的头部文本。

        :param always_block: 当前 always 过程块或极端兜底对象。
        :return: 可安全参与关键字匹配的头部文本。
        """

        # 正常路径优先读取 AlwaysBlock.header，避免把 dataclass 误当 dict 使用。
        if isinstance(always_block, AlwaysBlock):

            # AlwaysBlock.header 已是 formatter 归一后的敏感列表头文本。
            return always_block.header or ""

        # 兼容极端兜底对象：若外部调用传入带同名属性的对象，仍尽量读取 header。
        str_header = getattr(always_block, "header", "")  # 兜底对象上可能携带的 header 属性文本

        # 头部字段存在时直接返回，避免退化成对象 repr 噪声。
        if str_header:

            # 兜底对象显式提供 header 时，仍按该文本继续参与关键字匹配。
            return str(str_header)

        # 最后兜底保留字符串对象自身文本，供关键字搜索使用。
        return always_block if isinstance(always_block, str) else ""

    # 从 module 名和端口名里提取常见协议或适配提示词。
    def _module_protocol_hint(self, str_module_name: str, list_ports: list[PortDecl]) -> str:
        """
        返回当前 module 的协议或接口提示词。

        :param str_module_name: 当前 module 名称。
        :param list_ports: 当前 module 端口列表。
        :return: 命中时返回协议/接口提示词，否则返回空字符串。
        """

        # 全量搜索空间统一转小写，便于大小写无关匹配协议关键词。
        str_search_space = " ".join([str_module_name, *[port.name for port in list_ports]]).lower()  # 协议提示词搜索空间

        # 协议关键词按更具体的优先级排序，先匹配总线协议，再匹配泛化适配语义。
        for str_keyword, str_label in (
            ("iic", "IIC"),
            ("i2c", "IIC"),
            ("axis", "AXIS"),
            ("axi", "AXI"),
            ...
            # 命中已知协议关键词时，直接返回对应提示词。
            if str_keyword in str_search_space:

                # 协议提示词交由上层继续拼成完整中文说明。
                return str_label

        # 没有明确协议时，再识别适配/桥接/接口类 module。
        if any(str_token in str_search_space for str_token in ("adapter", "bridge", "iface", "interface")):

            # 适配型 module 仍应强调接口属性，而不是简单重复模块名。
            return "适配"
