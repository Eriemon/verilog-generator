...
# PG 引擎是兼容 facade 唯一允许调用的 RTL 规则实现。
...
class StaticLintIssue:
    ...
    severity: str  # PG catalog 等级映射后的严重度

    # message 承载固定 PG 规则的人类可读诊断。
    message: str  # 当前 PG finding 或门禁级消息
    ...
    # source 区分 PG 引擎与其他质量门来源。
    source: str = "current_module_issue"  # 兼容默认诊断来源

    # code 迁移后只允许固定 PG 编号。
    code: str = "PG_ERROR"  # 实例化时写入 PG1001 至 PG1072
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        返回:
            包含严重级别、定位、来源和 PG 编号的字典。
        """

        # 字典不添加旧 lint 专属字段，code 始终承载固定 PG 编号。
        return {
            "severity": self.severity,  # 兼容 error 或 warning 严重度
            "message": self.message,  # PG 门禁的人类可读诊断
            "path": self.path,  # 违规 RTL 相对路径
            "line": self.line,  # 一基源码行号
            "source": self.source,  # 固定 PG 诊断来源
            "code": self.code,  # PG1001 至 PG1072 固定编号
        }

# lint_generated_rtl 把权威 PG 报告映射为历史对象列表。
def lint_generated_rtl(spec: dict[str, Any], root: Path) -> list[StaticLintIssue]:
    """通过固定 PG 引擎返回兼容格式的 RTL 诊断。
    ...
    返回:
        激活 PG 门禁中所有非通过结果对应的兼容诊断列表。
    """

    # strict 模式保证 WARNING 与 BLOCKER 都按旧 lint 预期返回。
    dict_report = run_rtl_pg_gate(root, spec=spec, strict=True)  # 权威固定 PG 执行报告

    # 输出列表只做形状转换，不重新判断 RTL 语义。
    list_issues: list[StaticLintIssue] = []  # 兼容调用方消费的 PG 诊断对象

    # reserved 和 passed 条目不会形成历史 lint issue。
    for dict_result in dict_report["pg_gate_results"]:
        ...
        str_severity = "error" if dict_result["level"] == "BLOCKER" else "warning"  # 当前 PG 严重度

        # 真实 finding 优先保留精确路径和行号。
        list_findings = list(dict_result.get("findings") or [])  # 当前 PG 的定位证据
        ...
        for dict_finding in list_findings:
            ...
            list_issues.append(
                StaticLintIssue(
                    ...
                    message=str(dict_finding.get("message") or dict_result.get("message") or "PG gate did not pass."),  # PG 诊断文本
                    path=str(dict_finding.get("path") or ""),  # finding 提供的 RTL 路径
                    line=int(dict_finding.get("line") or 1),  # finding 提供的一基行号
                    source="rtl_pg_gate",  # 权威规则来源标识
                    code=str(dict_result["gate_id"]),  # 当前固定 PG 编号
                )
            )
