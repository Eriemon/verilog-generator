...
# dataclass 定义不可变 tracing 上下文与聚合结果。
from dataclasses import dataclass

# Any 限定 formatter typed facts 的兼容边界。
from typing import Any

# cone、作用域目标和循环三态是 tracing 的公开值合同。
from .vg_comb_model import CombTargetCone, HierarchyGraph, LoopPresence, ScopedTarget

# 解冻入口只为当前调用建立事实副本。
from .vg_comb_model import FrozenFact, SpecializedModule, thaw_fact

# 静态引用提取保留位选并隔离动态选择。
from .vg_comb_selectors import base_target, reference_targets

# hierarchy 查询入口提供 endpoint producer 与冻结 binding。
...
def _trace_function_call(
    ...
    if any(str_item.startswith(f"{str_callee}@") for str_item in context.function_call_stack):
    ...
    # formal 声明按 position 建立名称到 actual tree 的绑定。
    list_formals = list(dict_function_fact.get("formals", []) or [])  # 当前函数有序 formal 目录

    # 仅绑定同时存在 formal 与 typed actual 的位置。
    dict_function_bindings = {  # 当前函数 formal 到调用方 actual typed tree
        str(dict_formal.get("name") or ""): list_actual_expressions[int_position]  # 当前位置 formal actual
...
def _trace_expression(
    ...
    if str_kind == "identifier":
        ...
        # 函数 formal 先代换为调用方 actual tree。
        if str_reference in dict_bindings:

            # actual 已在进入函数体前按调用方上下文完整追踪，此处仅表示值复用。
            return _freeze_accumulator(accumulator)

        # 普通信号引用按当前模块作用域递归 tracing。
        if str_reference:
    ...
    if str_kind == "select":
        ...
        for str_reference in reference_targets(expression):
            ...
            # select 基础端点的上游结果进入当前表达式。
            _merge_result(accumulator, trace_result_result_reference)

        # selector 索引子树不重复作为独立数据 producer 遍历。
        return _freeze_accumulator(accumulator)
...
def trace_reference(
    ...
    if tuple_actual is not None:
        ...
        module_parent = _module_for_target(  # actual 求值所需的 parent occurrence 特化模块
            context.graph,  # parent actual 所在的层次绑定图
...
def _trace_storage_projection(
    ...
    for frozen_fact in module.storage_drivers:
        ...
        # 解冻 storage fact 读取 target、expression 和 controls。
        dict_fact = _mapping(frozen_fact)  # 当前候选 storage 驱动事实

        # 其他 Q target 与当前 synthetic endpoint 无关。
        if str(dict_fact.get("target") or "").replace(" ", "") != str_storage_target:

            # 继续检查当前模块后续 storage facts。
            continue
    ...
    if not any(
        str(_mapping(item).get("target") or "").replace(" ", "") == str_storage_target
        for item in module.storage_drivers
    ):
...
# 作用域目标递归入口实现 Q cut、本地 producer、实例 output 与 unknown 边界。
def _trace_scoped_target(context: TraceContext) -> TraceResult:
    ...
    module = _module_for_target(context.graph, context.current)  # 当前目标所属特化模块
    ...
    # 本地组合事实按目标精确匹配，基础引用可汇聚静态位驱动。
    list_local_facts = [  # 当前 endpoint 对应的本地组合驱动事实
        _mapping(frozen_fact)  # 解冻单条组合事实供当前 trace 读取
        for frozen_fact in module.comb_expressions  # 遍历当前 occurrence 全部本地驱动
        if str(_mapping(frozen_fact).get("target") or "").replace(" ", "") == context.current.target  # 精确端点
        or base_target(str(_mapping(frozen_fact).get("target") or "")) == context.current.target  # 整向量汇聚静态位
    ]

    # 每条本地驱动独立贡献 operation 和数据依赖。
    for dict_fact in list_local_facts:
    ...
    for producer_ref in drivers_for_endpoint(context.graph, context.current):
        ...
        # unresolved boundary 和所有 unknown producer 均只污染当前 endpoint。
    ...
    if list_local_facts and module.loop_presence == "present":
...
# 公开 target 入口把 TraceResult 适配为带完整身份的 CombTargetCone。
def trace_target_cone(
    ...
    # 公共入口使用空调用栈、空循环身份和空 visiting 集合开始。
    context = TraceContext(graph, target)  # 当前目标的初始 tracing 上下文

    # 内部递归结果保留已知操作下界与局部 unknown。
    trace_result_result = _trace_scoped_target(context)  # 当前目标跨层 tracing 结果

    # 目标所属模块决定用户可见 module 名和最早驱动行。
    module = _module_for_target(graph, target)  # 用户可见目标定位所需的特化模块

    # 缺失 occurrence 时仍使用 specialization implementation 提供定位。
    obj_identity = target.specialization.implementation  # 当前目标模块实现身份

    # 本地事实最早行用于 finding 源码定位。
    list_lines = [  # 当前目标本地驱动的有效源码行
        int(dict_fact.get("line") or 1)  # 单条匹配事实的一基行号
        for dict_fact in (_mapping(item) for item in module.comb_expressions)  # 解冻本地表达式事实
        if str(dict_fact.get("target") or "").replace(" ", "") == target.target  # 只定位当前静态目标
    ] if module is not None else []  # graph 缺失 occurrence 时没有本地行号

    # 无本地驱动的实例 output 使用模块定义起始行作为稳定回落。
    int_line = min(list_lines) if list_lines else obj_identity.definition_span.line_start  # 当前 cone 用户可见行号
    ...
    return CombTargetCone(
        ...
        definition_root=graph.root.identity,
