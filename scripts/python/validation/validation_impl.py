...
def validate_generated(request: ValidationRequest) -> ValidationReport:
    ...
    list_reference_cases = _reference_case_ids(request.semantic_contract) or _collect_reference_cases(path_root)  # 需要在 testbench 中覆盖的 reference case
    ...
    # list_tb_cases 保留 semantic contract 中声明的测试用例。
    list_tb_cases = list_reference_cases  # reference 合同要求覆盖的用例编号

    # testbench 诊断在 readiness 前阻断合同漂移。
    list_testbench_issues = func_tb_gate(dict_tb_spec, path_root, list_tb_cases, request.semantic_contract)  # 仿真前的测试平台合同诊断
    ...
    tuple_semantic = _validate_semantic_execution(path_root, request.semantic_contract)  # reference 语义诊断和度量
...
# _path_is_existing_file 判断候选路径当前是否仍是可访问文件。
...
# _path_is_existing_dir 判断候选路径当前是否仍是可访问目录。
...
# _iter_existing_tree_paths 返回当前仍可访问的 artifact 树后代路径。
...
# _semantic_contract_requires_transcript 判断 reference 合同是否必须有 transcript。
...
# _reference_case_ids 读取显式 semantic contract case id。
