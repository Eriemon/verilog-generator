...
# 基于现有分析结果生成 improve_existing 的变换计划骨架。
def build_transform_plan(
    ...
    *,
    ...
    """为 improve_existing 工作流构建稳定的计划态合同。
    ...
    # 返回 improve_existing 计划态合同，供后续 wrapper、validation 与等价性流程复用。
    return {
        "version": 1,  # 变换计划合同版本号
        "mode": "improve_existing",  # 当前合同对应 existing RTL 改进模式
