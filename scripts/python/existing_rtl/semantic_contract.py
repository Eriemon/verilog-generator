"""构建 Python semantic 合同并比对 Verilog 语义 transcript。"""

# 延迟注解解析，避免导入期解析嵌套 JSON 类型。
from __future__ import annotations

# 标准库负责动态加载 semantic model、稳定序列化和合同哈希。
import hashlib
...
SEMANTIC_RESULT_TAG = "VERILOG-GEN-RESULT"  # Verilog transcript 语义结果标记

# audit_semantic_model 是 Python stage 产出 reference 合同的公开入口。
def audit_semantic_model(path: Path) -> dict[str, Any]:
    """审计 Python semantic model 并生成稳定语义合同。

    参数:
        path: semantic model 文件路径，或包含 model.py / *_model.py 的目录。

    返回:
        包含 case、输出键、checkpoint 键和 sha256 的 reference 合同字典。

    异常:
        semantic model 缺少必要 API、向量格式非法或输出不稳定时抛出 ValueError。
    """

    # path_root 用于在目录输入时解析向量文件和相对模型路径。
    path_root = path if path.is_dir() else path.parent  # 向量搜索和相对路径计算根

    # path_model 是实际加载的 Python semantic model 文件。
    path_model = _model_path(path)  # semantic model 文件路径
    ...
    # 缺少 run_tests 说明 semantic model 不满足最小审计合同。
    if not callable(func_run_tests):

        # 错误前缀保持 current-project 终端可见文本规范。
        raise ValueError("> ERR: [Python] Python semantic model must expose callable run_tests().")

    # run_case 必须可调用，否则无法为 case 生成期望输出。
    if not callable(func_run_case):

        # 错误消息保留 API 名称，便于模型或用户修复 reference 文件。
        raise ValueError("> ERR: [Python] Python semantic model must expose callable run_case(case).")
    ...
    for dict_case in _canonical_cases(list_vectors):
        ...
        if value_checkpoints_first is not None:
            ...
            # checkpoint 字段保持可选，兼容旧 semantic model。
    ...
    return {
        ...
        "model": _model_summary(path, path_root, path_model, func_collect_checkpoints),  # semantic model 摘要
...
def parse_semantic_transcript(text: str) -> dict[str, Any]:
    ...
    for str_raw_line in text.splitlines():
        ...
        # 没有 tag 的日志行不参与语义解析。
        if not str_line or SEMANTIC_RESULT_TAG not in str_line:

            # 跳过普通日志，继续扫描后续行。
            continue

        # str_payload_text 取出 tag 后面的 JSON 文本。
        str_payload_text = str_line.split(SEMANTIC_RESULT_TAG, 1)[1].strip()  # tag 后载荷文本
...
# compare_reference_to_transcript 比对 Python semantic 与 RTL transcript。
def compare_reference_to_transcript(semantic_contract: dict[str, Any], transcript: dict[str, Any]) -> dict[str, Any]:
    """比较 reference 合同和 transcript 语义结果是否一致。

    参数:
        semantic_contract: audit_semantic_model 生成或同形状的 reference 合同。
    ...
    list_reference_cases = [  # 过滤后的 reference case 对象
        ...
        for dict_item in semantic_contract.get("cases", [])  # 原始 reference case 序列
            ...
            list_mismatched_cases.append(
                {
                    ...
                    "expected_outputs": value_expected_outputs,  # Python semantic 输出快照
            ...
            list_checkpoint_drift.append(
                {
                    ...
                    "expected_checkpoints": value_expected_checkpoints,  # Python semantic 观测点
    ...
    list_order_drift = _case_order_drift(semantic_contract, transcript)  # case 顺序漂移列表
    ...
    return {
        ...
        "reference_sha256": semantic_contract.get("sha256"),  # Python semantic 审计哈希
        "transcript_sha256": transcript.get("sha256"),  # RTL 日志语义摘要哈希
    }

# _model_path 解析文件或目录输入对应的 semantic model 文件。
def _model_path(path: Path) -> Path:
    """定位可审计的 Python semantic model 文件。

    参数:
        path: 直接的 Python 文件路径，或待搜索 model.py / *_model.py 的目录。

    返回:
        唯一的 Python semantic model 文件路径。

    异常:
        目录内找不到模型或找到多个候选时抛出 ValueError。
    """

    # 文件输入直接作为 semantic model。
    if path.is_file():
    ...
    if not list_candidates:
        ...
        # 错误提示保留 Python semantic model 关键词。
        raise ValueError("> ERR: [Python] No Python semantic model file was found.")
    ...
    if len(list_candidates) > 1:
        ...
        # 调用方应传入明确文件路径或清理候选。
        raise ValueError(
            "> ERR: [Python] Multiple Python semantic model files were found; audit-semantic expects exactly one."
        )

    # 返回唯一候选文件。
    return list_candidates[0]

# _load_module 使用唯一模块名加载 semantic model。
def _load_module(path_model: Path) -> types.ModuleType:
    """从文件路径动态加载 Python semantic module。

    参数:
        path_model: 待加载的 semantic model Python 文件。
    ...
    # str_module_name 避免不同 semantic model 之间模块名冲突。
    ...
    if config_module_spec is None or config_module_spec.loader is None:
        ...
        # 错误中包含路径，便于定位坏文件。
        raise ValueError(f"> ERR: [Python] Could not load Python semantic model from {path_model}.")
...
def _reference_vectors(module_type_reference: types.ModuleType, path_root: Path) -> list[Any]:
    """读取 semantic model 提供的输入向量列表。

    参数:
        module_type_reference: 已加载的 semantic model 模块。
    ...
    if not list_vector_paths:
        ...
        # 提示两种允许来源。
        raise ValueError(
            "> ERR: [Python] Python semantic model must provide REFERENCE_VECTORS or a vectors.json file."
        )
...
def _canonical_cases(list_vectors: list[Any]) -> list[dict[str, Any]]:
    ...
    参数:
        list_vectors: semantic model 或 vectors.json 提供的原始 case。
...
def _collect_checkpoints_once(func_collect_checkpoints: Any, dict_case: dict[str, Any]) -> Any | None:
    ...
    参数:
        func_collect_checkpoints: semantic model 中的 collect_checkpoints 函数或 None。
    ...
    if not callable(func_collect_checkpoints):
        ...
        # None 表示该 semantic model 未提供 checkpoint。
        return None
...
# _model_summary 构造 semantic model 审计摘要。
def _model_summary(
    path_input: Path,
    path_root: Path,
    path_model: Path,
    ...
    参数:
        path_input: audit_semantic_model 调用方传入的原始路径。
    ...
    dict_summary = {  # reference 合同里的模型来源摘要
        "path": str_model_path,  # semantic model 相对展示路径
...
def _case_order_drift(semantic_contract: dict[str, Any], transcript: dict[str, Any]) -> list[Any]:
    """计算 reference 和 transcript 的 case 顺序漂移。

    参数:
        semantic_contract: reference 合同字典。
    ...
    list_order_drift = [  # reference 顺序中被 transcript 打乱的位置
        ...
        for obj_case_id, obj_observed in zip(  # 同索引比较 reference 与 transcript
            semantic_contract.get("case_ids", []),  # reference 合同 case_id 顺序
