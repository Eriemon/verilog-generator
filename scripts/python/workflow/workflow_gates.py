...
from scripts.python.existing_rtl.verifier import verify_stage
...
def _semantic_gate(
    ...
    # semantic_contract 来自 Python stage 的可执行参考合同。
    semantic_contract = stage_outputs.get("python", {}).get("semantic_contract")  # Python 参考语义合同

    # validation metrics 缺失时语义门禁不适用。
    if not semantic_contract or not validation_report.metrics:
    ...
    dict_gate_result = _semantic_gate_result(plan, semantic_contract, validation_report, dict_semantic_metrics)  # 语义门禁结果
    ...
    # trace 事件记录 semantic contract 和语义验证摘要。
    _append_verify_trace(
        trace_path,
        attempt_dir,
        output_path=path_gate_result,
        from_contract=stage_outputs.get("python", {}).get("contract_paths", {}).get("semantic_contract"),
...
def _semantic_gate_result(
    ...
    参数:
        plan: 当前 attempt 使用的工作流计划。
        semantic_contract: Python 阶段生成的语义参考合同。
...
def _semantic_compatibility_result(plan: dict[str, Any], semantic_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    # summary 保留从 Python semantic 到当前验证指标的比较方向。
    dict_summary = {
