"""整理 semantic vector 的稳定语义摘要，供提示词和验证链路比对。"""
...
VECTOR_HASH_TAG = "VERILOG-GEN-VECTORS-SHA256:"  # semantic vector 哈希标记

# 公开文件入口负责把磁盘 JSON 纳入稳定契约生成流程。
def audit_vectors(vectors_path: Path) -> dict[str, Any]:
    """读取 semantic vector JSON 文件并生成稳定契约。

    参数:
        vectors_path: 指向 semantic vector JSON 的本地路径。
...
def vector_contract_from_payload(payload: Any, *, source: str | None = None) -> dict[str, Any]:
    """把 semantic vector 负载转换成可复现的语义契约。
    ...
    tuple_normalized_cases = tuple(  # JSON 字段乱序仍生成同一 sha256
        ...
        for raw_case in list_cases  # 原始 semantic vector 用例
    )
...
# 目录扫描入口为接口契约检查批量收集 semantic vector 摘要。
def find_vector_contracts(root: Path) -> list[dict[str, Any]]:
    """扫描目录下的 semantic vector 文件并生成契约列表。
...
def _cases_from_payload(payload: Any) -> list[Any]:
    """从兼容的 semantic vector 负载中提取 case 列表。
    ...
    # 非 dict 负载沿用历史约定，直接把自身视为 case 序列候选。
        ...
        # list 负载本身就是 semantic vector 的 case 序列。
...
def _case_sort_key(vector_case: Any) -> str:
    """生成 semantic vector case 的稳定排序键。
...
def _keys_for(list_cases: list[Any], tuple_candidate_fields: tuple[str, ...]) -> list[str]:
    """收集 semantic vector 输入或输出侧可见字段名。

    参数:
        list_cases: 已排序的 semantic vector case 列表。
