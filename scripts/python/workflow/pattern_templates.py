...
# 模式模板目录位于 skill assets 内，随发布包一起分发。
TEMPLATE_ROOT = skill_root() / "assets" / "verilog_pattern_templates"  # 模式模板资产目录

# catalog 记录模板 id、选择条件和正文文件相对路径。
CATALOG_PATH = TEMPLATE_ROOT / "catalog.json"  # 模式模板 catalog 路径

# 公开读取器加载模式模板 catalog。
def load_pattern_template_catalog() -> dict[str, Any]:
    """读取并校验Verilog 模式模板 catalog。
    ...
    dict_payload = json.loads(CATALOG_PATH.read_text(encoding="utf-8"))  # 模式模板 catalog 载荷

    # 顶层必须是 JSON 对象，才能读取 templates 字段。
    if not isinstance(dict_payload, dict):

        # 错误消息保留原始关键信息，并补齐项目脚本前缀。
        raise ValueError(f"> ERR: [Python] Pattern template catalog must be a JSON object: {CATALOG_PATH}")
    ...
    if not isinstance(list_templates, list) or not list_templates:
        ...
        raise ValueError("> ERR: [Python] Pattern template catalog must contain a non-empty templates list.")
...
def list_pattern_templates() -> list[dict[str, Any]]:
    ...
    return [copy.deepcopy(item) for item in load_pattern_template_catalog()["templates"]]

# 公开选择器按规格或 existing RTL analysis 选择模式模板。
def select_pattern_templates(spec: dict[str, Any]) -> list[dict[str, Any]]:
    """选择适用于当前规格的模式模板并加载正文。
    ...
    list_results: list[dict[str, Any]] = []  # 已选择模式模板列表

    # catalog 顺序决定输出模板顺序，避免规则匹配顺序影响 prompt 稳定性。
    for dict_template in load_pattern_template_catalog()["templates"]:
...
def summarize_pattern_templates(spec: dict[str, Any]) -> list[dict[str, Any]]:
    """返回当前规格命中的模式模板摘要。
    ...
    return [
        ...
        for item in select_pattern_templates(spec)
    ]
...
def _match_from_analysis(analysis: dict[str, Any]) -> list[str]:
    """从 existing RTL analysis 中匹配模式模板 id。
