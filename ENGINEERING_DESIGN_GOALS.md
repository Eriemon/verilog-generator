...
- Pipeline: run fixed stages in order for generation, plus controlled analysis/improvement/verify-repair subflows for existing RTL.
...
- Keep helper scripts optional: `scripts/python/quality/verilog_lint.py` and `scripts/python/generation/tb_generator.py` must never become mandatory prerequisites for the main workflow.
...
- Smoke tests cover successful generation workflow execution, existing-RTL analysis/improvement/verify-repair flows, merge-assist planning, target rejection, prompt cleanliness, and Verilog artifact validation.
