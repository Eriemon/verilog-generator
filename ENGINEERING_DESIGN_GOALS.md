...
The design goal is not to be a general hardware generator. It is to help Codex reliably turn a confirmed hardware specification into synthesizable Verilog RTL, analyze existing RTL into stable contracts and design explanations, emit a self-checking testbench, drive a bounded verify-repair loop for existing RTL, support wrapper-first merge assist, and preserve validation evidence that can be inspected by a human or host application.
...
- Pipeline: run fixed stages in order for generation, plus controlled analysis/refinement subflows for existing RTL.
