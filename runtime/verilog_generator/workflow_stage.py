...
def _augment_python_stage_output(
    ...
    dict_reference_contract: dict[str, Any] = audit_reference(paths.artifact_dir)  # Python 参考模型合同

    # reference_contract_path 是语义门读取的固定证据文件。
    reference_contract_path = paths.stage_dir / "reference_contract.json"  # 参考合同路径

    # 将参考合同写入 stage 目录，便于 release 证据收集。
    write_json(reference_contract_path, dict_reference_contract)
...
def _attach_python_contracts(
    ...
    参数:
        ...
        reference_contract: Python 参考模型审计合同。
        reference_contract_path: 参考模型合同 JSON 路径。
    ...
    stage_output["reference_contract"] = reference_contract  # semantic gate 读取的 Python 参考合同对象
