...
CONFIG_DIR = Path(__file__).resolve().parents[2] / "config"  # skill 内置配置目录
...
def _source_repository_root() -> Path | None:
    ...
    # docs、smoke 和 tests 共同构成源码仓库治理标记。
    tuple_repo_markers = (
        path_candidate_root / "docs" / "handoff" / "HANDOFF.md",  # 源码仓库交接文档标记
        path_candidate_root / "smoke" / "run_smoke.py",  # 源码仓库 smoke 根脚本标记
    ...
    # docs/smoke/tests 任一标记缺失都视为安装态或不完整副本。
