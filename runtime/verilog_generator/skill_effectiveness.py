...
# 集成 facade 的 routing 与 workflow 入口。
from integration.verilog_adapter import (
...
# 质量门和 verify-repair facade 覆盖最终工件验证与已有 RTL 修复。
from integration.verilog_adapter import (
...
from .rtl_md_constraints import (
...
# 默认生成链路 case 验证 use-case/refined-template 注入效果。
def _evaluate_default_generation_case(
    ...
    dict_with_checks = _expectation_checks(  # 启用模板后的命中结果
        ...
        refined_templates=dict_requirements.get("selected_refined_template_ids", []),  # 已选精炼模板列表
    ...
    dict_baseline_checks = _expectation_checks(  # baseline 对照组的命中结果
        ...
        refined_templates=[],  # baseline 不注入精炼模板
    ...
    dict_with_skill = {
        ...
        "selected_refined_template_ids": dict_requirements.get("selected_refined_template_ids", []),  # 选中的精炼模板
    ...
    dict_without_skill = {
        ...
        "selected_refined_template_ids": [],  # baseline 恒为空的精炼模板集合
    ...
    return {
        ...
        "refined_templates": summarize_refined_templates(dict_spec),  # 规格命中的精炼模板摘要
    }
...
def _evaluate_transform_case(
    ...
    dict_checks = _enabled_checks(  # transform 回归要启用的检查集合
        ...
        dict_expectations,  # 仅启用 style_refine case 明确声明的检查项
    )
...
# style_refine case 验证 Markdown 报告和 ready 状态。
def _evaluate_style_refine_case(
    case: dict[str, Any],
    str_case_id: str,
    temp_root: Path,
) -> dict[str, Any]:
    """评估 style_refine 辅助是否产出可审查报告。

    参数:
        case: style_refine 回归配置，提供输入 RTL 和章节期望。
        str_case_id: 当前 style_refine 回归的稳定标识。
        temp_root: 当前整轮评估共享的临时根目录。
    ...
    # 解析 style_refine 回归要读取的 RTL 夹具。
    path_source = SKILL_ROOT / str(case["source"])  # style_refine 输入 RTL 夹具

    # 为 style_refine 输出准备独立目录，便于单独核对 Markdown 报告。
    path_case_root = _prepare_case_root(temp_root, str_case_id)  # style_refine 隔离工件目录

    # style_refine 只生成辅助报告，不直接改写源文件。
    dict_result = refine_existing_verilog(  # style_refine 报告生成结果
        path_source,  # 需要审阅风格的 RTL 源文件
        out_dir=path_case_root / "style-refine",  # style_refine Markdown 的输出目录
        refine_goal="style_refine",  # 仅生成风格审阅报告
    )
    ...
    # 读取 style_refine 的 validation 记录，确认 ready 标记与 goal 没有漂移。
    dict_validation = _read_json(  # 核对 style_refine ready/goal 的 validation 记录
        Path(dict_result["transform_validation_path"])  # ready 状态对应的 validation 文件路径
    )

    # 读取 style_refine case 自己声明的章节与 ready 约束。
    dict_expectations = _case_expectations(case)  # style_refine case 打开的断言集合

    # style_refine 需要同时覆盖报告章节和 ready 状态一致性。
    dict_checks = _enabled_checks(  # style_refine 报告章节与 ready 一致性检查
        {
            ...
            "suggested_refinements_present": "## Suggested style refinements" in str_style_text,  # 润色建议章节是否存在
            "ready_state_recorded": (  # ready 标记与目标模式的一致性检查
                bool(dict_validation.get("ready"))  # validation 是否记录 ready 标记
                and dict_validation.get("goal") == "style_refine"  # goal 字段是否仍指向 style_refine
            ),  # ready 标记与 goal 是否同时保持一致
        },
        dict_expectations,  # 仅启用 style_refine case 显式要求的章节与 ready 检查
    )
...
def _evaluate_merge_assist_case(
    ...
    dict_result = refine_existing_verilog(  # merge_assist 的 planned 工件总表，供 merge_plan_present 等四个 merge_* 断言统一取路径
        path_source,  # merge_assist 要审阅的 existing RTL 输入源文件
        out_dir=path_case_root / "merge",  # merge_assist 计划包的输出目录
        refine_goal="merge_assist",  # 仅生成合并辅助工件
    )  # merge_assist 执行结果
...
def _evaluate_optimize_case(
    ...
    dict_without_candidate = refine_existing_verilog(  # 无候选输入时的 optimize 执行结果
        path_source,  # 无候选路径的原始 RTL 输入
        out_dir=path_case_root / "opt",  # 无候选路径输出目录
        refine_goal="optimize_assist",  # 强制走 optimize_assist 的候选对照分支
    ...
    dict_with_candidate = refine_existing_verilog(  # 带候选输入时的 optimize 执行结果
        path_source,  # 与无候选路径复用同一 RTL 输入
        out_dir=path_case_root / "opt-with-candidate",  # 有候选路径输出目录
        refine_goal="optimize_assist",  # optimize 辅助模式
    ...
    dict_checks.update(  # provenance 相关的兜底检查项
        {
            ...
            "reference_workspace_absent": "IC-" + "AGENT-HUB"
...
CASE_EVALUATORS: dict[str, CaseEvaluator] = {  # 供 _evaluate_case 按 kind 查表分派的 evaluator 注册表
    ...
    "style_refine_regression": _evaluate_style_refine_case,  # 核对 style_refine 报告与 ready 状态
