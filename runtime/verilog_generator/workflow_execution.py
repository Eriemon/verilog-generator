...
def _validate_attempt(
    ...
    # reference_contract 来自 Python stage，用于 RTL 语义比对。
    reference_contract = stage_state.stage_outputs.get("python", {}).get("reference_contract")  # Python 参考合同
    ...
    validation_report = validate_generated(  # validate_generated 返回的本地检查报告
        ...
        reference_contract=reference_contract,  # RTL 语义比对用的 Python 合同
    )
...
def _attempt_passed(validation_artifacts: ValidationArtifacts, gate_artifacts: GateArtifacts) -> bool:
    ...
    # 两者都通过才视为 workflow passed。
    return bool_validation_ok and bool_gate_ready
