...
from .validation import READINESS_LEVELS
from .verify_repair import AUTOMATION_MODES, TB_LANGUAGES, TB_MODES
    ...
    return (
        CommandSpec(
            ...
            (
                ...
                arg("--reference-contract", type=Path, help="Optional Python semantic reference contract JSON."),
            ),
            trace=True,
            state=True,
        ),
...
def _audit_command_specs() -> tuple[CommandSpec, ...]:
    ...
    return (
        CommandSpec(
            "audit-vectors",
            "Create a semantic contract from reference vectors JSON.",
            "audit-vectors",
        ...
        CommandSpec(
            "audit-reference",
            "Execute a Python reference model and emit a semantic reference contract.",
            "audit-reference",
...
def _feedback_command_specs() -> tuple[CommandSpec, ...]:
    ...
    # 固定既有 RTL 入口顺序，保留 analyze/refine/compare/verify 的使用路径。
    return (
        CommandSpec(
            "reflect",
...
def _existing_rtl_command_specs() -> tuple[CommandSpec, ...]:
    ...
    return (
        ...
        CommandSpec(
            "refine-existing",
            "Create a controlled refinement plan for existing RTL.",
            "refine-existing",
            (
                ...
                arg("--reference-artifacts-dir", type=Path),
                arg("--readiness", choices=READINESS_LEVELS, default="static"),
        ...
        CommandSpec(
            ...
            "compare-semantics",
            (
                arg("--reference", required=True, type=Path),
...
def build_cli_parser(command_handlers: Mapping[str, CommandHandler]) -> argparse.ArgumentParser:
    ...
    # 版本参数必须保持旧 CLI 文本，避免安装后 smoke 断言漂移。
    argument_parser_command_parser.add_argument("--version", action="version", version="erie-verilog-gen 0.3.6")
