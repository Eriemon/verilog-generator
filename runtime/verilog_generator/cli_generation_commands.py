...
from .quality_gate import run_verilog_quality_gate, write_quality_gate_report
...
def cmd_prompt(args: argparse.Namespace) -> int:
    ...
    # vector contract 可选注入 Python reference 语义合同。
    ...
    str_prompt_output = render_prompt(  # 组合规格、证据和记忆后的模型提示词
        ...
        vector_contract=dict_vector_contract,  # reference model 提供的向量约束
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    # reference_contract 可选提供语义参考检查。
    dict_reference_contract = read_json(args.reference_contract) if args.reference_contract else None  # 参考语义合同
    ...
    validation_report = validate_generated(  # 汇总静态、接口和可选外部验证的报告对象
        ...
        reference_contract=dict_reference_contract,  # reference model 提供的语义对照合同
    )
