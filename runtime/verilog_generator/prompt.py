...
# 精炼模板和 reference tag 注入 prompt，供后续 RTL/testbench 对齐。
...
def _render_rtl_prompt(dict_spec: dict[str, Any], dict_options: dict[str, Any]) -> str:
    ...
    dict_base_context = {  # 把一次性 RTL 规格、manifest 和模板正文合并给主提示词
        ...
        "refined_templates": _refined_template_context(dict_spec),  # 本地 RTL 参考模式摘要
    }
...
def _base_prompt(dict_context: dict[str, Any]) -> str:
    ...
    # str_refined_template_text 展示从仓库 RTL 中蒸馏出的设计模式。
    str_refined_template_text = _format_refined_template_section(dict_context.get("refined_templates"))  # 精炼模板段
...
def _render_staged_prompt(dict_spec: dict[str, Any], str_stage: str, dict_options: dict[str, Any]) -> str:
    ...
    dict_render_context = {  # 绑定 Markdown 渲染器逐章读取的键名、裁剪载荷、记忆约束和模板片段
        ...
        "refined_templates": _refined_template_context(dict_scoped_plan),  # 仓库 RTL 模式段落引用的精炼模板摘要
    }
...
def _staged_prompt_text(dict_context: dict[str, Any]) -> str:
    ...
    # str_refined_template_text 提供可借鉴但不能照抄的本地 RTL 结构。
    str_refined_template_text = _format_refined_template_section(dict_context.get("refined_templates"))  # 精炼模式说明
...
def _stage_guidance(
    ...
    参数:
        ...
        str_stage: 已校验的阶段名称。
        str_comment_language: 已校验的注释语言代码。
        dict_vector_contract: 可选 reference vector 合同。
    ...
    # python 阶段生成可执行 reference model。
    if str_stage == "python":

        # 返回 Python reference 阶段说明。
...
def _python_stage_guidance(
    ...
    """返回 Python reference 阶段的 prompt 说明。
    ...
    # str_design_name 用于构造 reference vector 文件路径。
    str_design_name = str(dict_spec["name"])  # 设计名称

    # list_rules 保留 reference model 的可执行 API 合同。
    list_rules = [  # reference model 必须暴露给语义门禁的行为合同
        ...
        "Avoid external dependencies and random behavior unless seeded deterministically.",  # reference model 必须可离线复现
    ]
...
def _rtl_stage_guidance(
    ...
    # str_design_name 用于指向前序 Python reference model。
    str_design_name = str(dict_spec["name"])  # reference model 文件名使用的设计标识
    ...
    list_rules = [  # RTL 产物必须满足的 oracle 和可综合性规则
        ...
        "Mirror the Python reference model's observable behavior.",  # RTL 输出必须对齐 oracle 可观测结果
        ...
        (
            ...
            "that mirrors the Python reference model's verification vectors."
        ),
        (
            f"The testbench must emit one machine-readable transcript line per case using "
            f"the prefix `{REFERENCE_RESULT_TAG}` followed by one JSON object."
        ),
    ]

    # 向量合同规则只有在 Python reference 阶段已经产出合同时才追加。
...
def _stage_files(dict_spec: dict[str, Any], str_stage: str) -> list[dict[str, Any]]:
    ...
    # Python 阶段需要同时产出 reference model 和向量 JSON。
    if str_stage == "python":

        # 返回 Python reference 双文件合同。
        return [
...
def _comment_rules_for(str_target: str, str_comment_language: str) -> list[str]:
    ...
    return [
        ...
        (
            ...
            "and testbench constructs must use `references/verilog-comment-placement.md`."
        ),
        (
...
def _design_requirements_context(dict_spec: dict[str, Any]) -> dict[str, Any]:
    ...
    # list_refined_templates 是精炼 RTL 模板摘要。
    list_refined_templates = _refined_template_context(dict_spec)  # 精炼模板摘要列表
    ...
    return {
        ...
        "selected_refined_template_ids": [dict_item["template_id"] for dict_item in list_refined_templates],  # 精炼模板 id
        "refined_templates": list_refined_templates,  # 精炼模板摘要
...
# _refined_template_context 汇总精炼 RTL 模板。
def _refined_template_context(dict_spec: dict[str, Any]) -> list[dict[str, Any]]:
    """汇总当前规格适用的精炼 RTL 模板。
    ...
    返回:
        summarize_refined_templates 返回的模板摘要列表。
    """

    # 返回精炼模板摘要，具体选择逻辑由 refined_templates 模块维护。
    return summarize_refined_templates(dict_spec)
...
# _format_refined_template_section 渲染精炼 RTL 模板小节。
def _format_refined_template_section(list_refined_templates: list[dict[str, Any]] | None) -> str:
    """渲染精炼 RTL 设计模式 prompt 小节。

    参数:
        list_refined_templates: summarize_refined_templates 返回的模板摘要列表。
    ...
    # 没有精炼模板时不输出该小节。
    if not list_refined_templates:

        # 没有精炼模板时不向模型提供额外设计范式。
        return ""

    # str_metadata_json 展示精炼模板摘要列表。
    str_metadata_json = json.dumps(list_refined_templates, indent=2, ensure_ascii=False)  # 精炼模板索引 JSON

    # str_template_sections 渲染每个模板的 Verilog 代码块。
    str_template_sections = _refined_template_sections(list_refined_templates)  # 精炼模板代码段
...
# _refined_template_sections 读取并渲染精炼模板正文。
def _refined_template_sections(list_refined_templates: list[dict[str, Any]]) -> str:
    """读取精炼模板文件并渲染为 Verilog 代码块。
    ...
    list_sections: list[str] = []  # 精炼模板 Markdown 小节

    # 逐个读取模板文件，保持旧版 prompt 内嵌正文行为。
    for dict_template in list_refined_templates:

        # path_template 是本地精炼模板文件路径。
        path_template = Path(str(dict_template["path"]))  # 精炼模板文件路径

        # str_content 是模板文件正文。
        str_content = path_template.read_text(encoding="utf-8").rstrip()  # 精炼模板正文
...
def _vector_contract_rules(dict_vector_contract: dict[str, Any] | None) -> list[str]:
    """返回 reference vector 合同对 RTL testbench 的约束。

    参数:
        dict_vector_contract: Python reference 阶段生成的向量合同或 None。
