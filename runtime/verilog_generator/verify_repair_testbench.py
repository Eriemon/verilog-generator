...
# refine_existing_rtl 负责生成标准 testbench scaffold。
from .existing_rtl_refinement import RefineExistingRtlOptions, refine_existing_rtl
...
def materialize_tb_contract(
    ...
    **dict_tb_options: Any,
    ...
    # generate 模式复用 existing RTL refinement 的 TB scaffold。
...
def _generate_testbench_contract(
    *,
    ...
    # spec 和语言参数直接约束 refinement 生成的 scaffold。
    ...
    refine_existing_rtl_options_config = RefineExistingRtlOptions(  # 生成式 testbench scaffold 的 refine 配置
    ...
    # refinement 入口会写出 tb_scaffold/tb/tb_<module>.v。
    refine_existing_rtl(
        Path(dict_analysis["provenance"]["source_paths"][0]),
        out_dir=path_out_dir / "tb_scaffold",
        refine_goal="tb_scaffold",
        options=refine_existing_rtl_options_config,
    )

    # 生成出的 TB 位于 refinement 固定目录。
    path_generated_tb = path_out_dir / "tb_scaffold" / "tb" / f"tb_{str_module_name}.v"  # refinement 固定输出的 TB 文件
