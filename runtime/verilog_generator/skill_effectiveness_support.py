...
# verify-existing facade 用于 patch-library 评估分支。
from integration.verilog_adapter import verify_existing_verilog
...
def _expectation_checks(
    *,
    ...
    """根据 eval expectations 检查 prompt、requirements 和 codegen plan。
        ...
        :param refined_templates: 进入 prompt 的 refined template id 序列。
    ...
    # refined template id 列表必须保持顺序一致。
    if "selected_refined_template_ids" in expectations:

        # 复制期望列表，避免外部对象被后续逻辑修改。
        list_expected_ids = list(expectations["selected_refined_template_ids"])  # 期望 refined template id 序列

        # refined template 顺序影响 prompt 生成内容，必须精确一致。
        bool_refined_ids_ok = list(refined_templates) == list_expected_ids  # refined template id 顺序检查结果

        # refined id 顺序结果写入报告，覆盖模板选择的顺序语义。
        dict_checks["selected_refined_template_ids"] = bool_refined_ids_ok  # refined template id 顺序是否正确

    # 检查 prompt 中是否按要求带有 refined 模板段。
    if "requires_refined_template_section" in expectations:

        # evals.json 决定 refined 模板段是否必须出现在 prompt。
        bool_refined_section_expected = bool(expectations["requires_refined_template_section"])  # 是否要求 refined 模板段

        # prompt 中的 refined 模板段存在性必须和期望一致。
        bool_refined_section_ok = ("## Refined Verilog design patterns" in prompt) is bool_refined_section_expected  # refined 模板段存在性是否符合期望

        # 该字段证明 refined 设计模式是否真正进入 prompt。
        dict_checks["requires_refined_template_section"] = bool_refined_section_ok  # refined 设计模式段落注入状态
    ...
    dict_report.update(
        {
            ...
            "refined_templates": [],  # 非默认 case 不计算模板摘要
        }
    )
...
# RTL Markdown fixture spec 与 tests/test_rtl_md_constraints.py 保持一致。
