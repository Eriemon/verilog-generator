...
def validate_generated(
    ...
    **kwargs: Any,
    ...
    :param kwargs: 兼容旧调用方的关键字选项，包括 run_external、readiness、
        comment_language、reference_contract、simulator_config 和 strict_generated_comments。
    ...
    # reference_contract 用于比对 Python reference 与 transcript。
    dict_reference_contract = kwargs.pop("reference_contract", None)  # 语义合同配置
    ...
    try:
        ...
        validation_request_validation_request: _impl.ValidationRequest = _impl.ValidationRequest(  # validation_impl 内部执行请求
            ...
            reference_contract=dict_reference_contract,  # reference 语义合同
...
# _simulator_config 保持 existing_rtl_refinement 的私有导入兼容。
...
# _select_simulator_backend 是测试和 refinement 依赖的可 patch 选择点。
...
def _install_patchable_backend_hooks() -> tuple[Any, Any, Any, Any]:
    ...
    # 保存当前工具映射入口，避免 refinement 查询被测试替身污染。
