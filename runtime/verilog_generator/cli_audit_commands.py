...
# cmd_audit_reference 检查 Python reference model 的向量合同。
def cmd_audit_reference(args: argparse.Namespace) -> int:
    """处理 audit-reference 子命令。
    ...
    # reference model 输入必须存在。
    path_artifact = require_workspace_path(args.path, purpose="reference model path", must_exist=True)  # 参考模型路径

    # reference audit 报告路径必须允许写入。
    path_output = require_write_path(args.out, purpose="reference audit output")  # 参考模型审计报告路径

    # audit_reference 统计可用测试 case 和合同摘要。
    dict_reference_contract = audit_reference(path_artifact)  # 参考模型审计合同

    # 写入 reference audit JSON。
    write_json(path_output, dict_reference_contract)

    # workflow-state 记录 reference audit 产物。
    record_state(args, "audit_reference", {"path": path_artifact, "output": path_output})
    ...
    if args.trace:
        ...
        dict_trace_event = {  # 参考模型审计 trace 事件
            "event": "audit_reference",  # 标识参考模型审计事件
            "path": path_artifact,  # 被审计的参考模型文件
            "output": path_output,  # 参考模型审计 JSON 输出
            "case_count": dict_reference_contract.get("case_count"),  # 参考 case 数量
        }
