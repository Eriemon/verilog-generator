...
class RefineExistingRtlOptions:
    ...
    参数:
        ...
        reference_artifacts_dir: 参考设计工件目录，可选。
        readiness: 当前验证准备度级别。
        tb_language: testbench 输出语言。

    返回:
        None。该对象只承载 refine_existing_rtl 的可选配置。
    """
    ...
    # 重新分析时优先从参考工件目录提取 RTL。
    reference_artifacts_dir: Path | None = None  # 参考设计工件目录
...
# 规范化并校验 refine_goal 参数，避免后续主流程分支走偏。
def require_refine_goal(goal: str) -> str:
    ...
    if str_normalized_goal not in REFINE_GOALS:

        # 用统一错误前缀返回可读的参数错误信息。
        raise ValueError(
            f"> ERR: [Python] Refine goal must be one of {', '.join(REFINE_GOALS)}."
        )

    # 返回已通过校验的规范化目标名。
    return str_normalized_goal

# 把可选 refine 配置补齐成完整对象，避免主流程重复处理 None 分支。
def _resolve_refine_options(
    options: RefineExistingRtlOptions | None,
) -> RefineExistingRtlOptions:
    """返回带默认值补齐后的 refine 配置对象。

    参数:
        options: 调用方传入的可选 refine 配置。

    返回:
        总是可直接读取字段的 refine 配置对象。
    """

    # 调用方未显式传配置时，构造一份默认 refine 配置。
    if options is None:

        # 返回补齐默认值后的 refine 配置对象。
        return RefineExistingRtlOptions()
...
def refine_existing_rtl(
    ...
    *,
    out_dir: Path,
    refine_goal: str,
    options: RefineExistingRtlOptions | None = None,
) -> dict[str, Any]:
    ...
    # 先把 refine_goal 规范成受支持的目标名。
    str_goal = require_refine_goal(refine_goal)  # 当前执行的精修目标名

    # 统一补齐 refine 可选配置，后续各分支都按完整对象取值。
    refine_existing_rtl_options_refine_options = _resolve_refine_options(options)  # 已补齐默认值的 refine 配置对象

    # 调用方可直接提供分析 JSON，避免重复跑结构分析。
    if refine_existing_rtl_options_refine_options.analysis_source is not None:

        # 先读出外部分析文件中的 JSON 文本。
        str_analysis_text = refine_existing_rtl_options_refine_options.analysis_source.read_text(encoding="utf-8")  # 外部分析文件中的 JSON 文本
    ...
    # 未提供分析 JSON 时，基于参考工件或源文件现算分析结果。
        ...
        # 先收集参考工件目录中的 RTL 输入。
        list_analysis_sources = _artifact_sources(refine_existing_rtl_options_refine_options.reference_artifacts_dir)  # 参考工件目录解析出的 RTL 输入列表
        ...
        str_spec_text = load_spec_text(refine_existing_rtl_options_refine_options.spec_source)  # 传给分析器的规格说明文本
    ...
    if str_goal == "optimize_assist":
        ...
        return _optimize_assist(
            ...
            candidate_artifacts_dir=refine_existing_rtl_options_refine_options.candidate_artifacts_dir,
            reference_artifacts_dir=refine_existing_rtl_options_refine_options.reference_artifacts_dir,
            readiness=refine_existing_rtl_options_refine_options.readiness,
        )
    ...
    if str_goal == "tb_scaffold":
        ...
        _write_tb_from_analysis(
            dict_analysis,
            path_tb,
            tb_language=refine_existing_rtl_options_refine_options.tb_language,
        )
    ...
    # style_refine 默认输出风格审查与精修引导报告。
    else:

        # 风格精修报告使用固定文件名，方便用户直接查阅。
        path_guide = out_dir / "style" / "style_refine_report.md"  # 风格精修报告输出路径

        # 输出可直接阅读的风格精修建议文档。
        _write_style_refine_guide(dict_analysis, path_guide)
...
def _optimize_assist(
    ...
    *,
    ...
    candidate_artifacts_dir: Path | None, reference_artifacts_dir: Path | None,
    readiness: str,
) -> dict[str, Any]:
    """生成优化辅助工件，并在候选 RTL 存在时附带语义比对结果。

    参数:
        source_path: 默认参考 RTL 路径，在缺少参考工件目录时作为回退输入。
        out_dir: optimize_assist 阶段的工件输出目录。
        analysis: 当前 RTL 的结构化分析结果。
        candidate_artifacts_dir: 候选 RTL 工件目录，可为空。
        reference_artifacts_dir: 参考 RTL 工件目录，可为空。
        readiness: 当前验证准备度级别。
    ...
    # 优先从参考工件目录取源文件，缺失时退回原始输入 RTL。
    list_reference_sources = _artifact_sources(reference_artifacts_dir)  # 参考工件目录解析出的 RTL 路径列表

    # 缺少参考工件时，退回 source_path 作为唯一参考 RTL。
    if not list_reference_sources:

        # 当前轮次没有参考工件目录内容时，退回到原始 RTL。
        list_reference_sources = [source_path]  # 回退后的参考 RTL 路径列表

    # 选出 optimize 流程实际使用的参考 RTL。
    path_reference_source = list_reference_sources[0]  # optimize 流程使用的参考 RTL 路径
    ...
    if path_candidate_source is not None:
        ...
        dict_compare_result = compare_semantics(  # compare 阶段产出的路径集合
            path_reference_source,  # compare 使用的参考 RTL 路径
    ...
    # 没有候选 RTL 时，仅保留参考设计上的 advisory 级 QoR 信息。
        ...
        dict_qor_report = _qor_report(  # 无候选 RTL 时退回生成的单边 advisory QoR 报告正文
            ...
            reference_path=path_reference_source,  # 单参考 advisory 场景使用的参考 RTL 路径
...
# 生成 style refine 指南，提醒调用方只做风格层精修而不改语义。
def _write_style_refine_guide(analysis: dict[str, Any], output_path: Path) -> None:
    """根据分析结果生成 style refine 指南文档。

    参数:
        analysis: RTL 结构化分析结果。
        output_path: style refine 指南输出路径。
    ...
    list_lines = [
        ...
        "",  # 标题与正文之间的空行
        f"Target module: `{analysis['module_info']['name']}`",  # style refine 面向的目标模块名
        "",  # 模块名与保留项标题之间的空行
        "## Preserve",  # 保留约束标题
        ...
        "",  # 保留项与建议项之间的空行
        "## Suggested style refinements",  # 风格建议标题
    ]  # style refine 指南正文行列表
